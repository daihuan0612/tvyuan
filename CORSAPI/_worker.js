...
async function getCachedJSON(url) {
  ...
  console.log(`Source type for URL ${url}: ${sourceType}`);
  ...
  console.log(`KV available: ${kvAvailable}`);
  ...
  if (kvAvailable) {
    ...
    // 如果没有缓存或缓存无效，从网络获取数据
    try {
      console.log(`Fetching data from network: ${url}`);
      const res = await fetch(url);
      console.log(`Fetch status for ${url}:`, res.status);
      
      if (!res.ok) {
        throw new Error(`Fetch failed: ${res.status} ${res.statusText} for URL: ${url}`);
      }
      ...
      // 缓存数据（10分钟）
      await KV.put(cacheKey, text, { expirationTtl: 600 });
      console.log(`Data cached for ${url}`);
      
      return data;
    } catch (fetchError) {
      console.error('Network fetch error:', fetchError.message);
      console.error('Response text preview:', fetchError.responseText ? fetchError.responseText.substring(0, 500) : 'N/A');
    }
  }
...
async function handleFormatRequest(formatParam, sourceParam, prefixParam, defaultPrefix) {
  try {
    ...
    // 过滤掉不健康的 API（disabled: true）
    const healthyData = {
      ...data,
      api_site: Object.fromEntries(
        Object.entries(data.api_site || {})
          .filter(([_, site]) => !site.disabled)
      )
    }
    
    const newData = config.proxy
      ? addOrReplacePrefix(healthyData, prefixParam || defaultPrefix)
      : healthyData
